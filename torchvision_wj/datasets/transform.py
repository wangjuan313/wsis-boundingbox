...
def transform_contour(transform, contour_x, contour_y):
    """ Apply a transformation to an axis aligned bounding box.
    """
    assert len(contour_x)==len(contour_y), "The x and y coordinates do not match."
    N = len(contour_x)
    points = transform.dot([contour_x,contour_y,[1]*N])

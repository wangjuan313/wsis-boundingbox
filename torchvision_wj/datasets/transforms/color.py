...
def color_aug_and_norm(meta, kwargs):
    ...
    if ('normalize' in kwargs)&isinstance(kwargs['normalize'],str):
        if kwargs['normalize']=='zscore':
            img = (img-img.mean())/img.std()
        elif kwargs['normalize']=='zero-one':
            img = (img-img.min())/(img.max()-img.min())
        elif kwargs['normalize']=='pos-neg-one':
    ...
    # cv2.waitKey(0)
    if ('normalize' in kwargs)& (not isinstance(kwargs['normalize'],str)):
